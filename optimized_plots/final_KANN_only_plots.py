...
from Model.Model_KANN import Trainer # Changed from Model_NN
from utils.util import eval_metrix

def get_args_for_trainer():
    parser = argparse.ArgumentParser('Hyper Parameters for KANN model') # Changed from NN
    ...
    parser.add_argument('--save_folder', type=str, default='optimized_plots/KANN_Final_Optimized_Results', help='save folder') # Changed
...
def plot_single_experiment_losses(experiment_folder, param_name, param_value):
    ...
    plt.title(f'KANN Sensitivity: {param_name} = {param_value} (Epoch Loss)', fontsize=16) # Changed
...
def analyze_and_plot_sensitivity_type(base_output_folder, sensitivity_type_folder, param_name):
    ...
    plt.title(f'KANN Sensitivity Analysis: Min Epoch Loss vs. {param_name}', fontsize=16) # Changed
    ...
    with open(summary_txt_path, 'w') as f:
        f.write(f"KANN Sensitivity Analysis for: {param_name}\n") # Changed
        f.write("--------------------------------------------------\n")
...
def train_final_optimal_nn_model(optimal_hyperparams, final_save_folder): # Renamed function to be more general
    print("\n--- Starting Final KANN Model Training with Optimal Hyperparameters ---") # Changed
    ...
    print("--- Final KANN Model Training Finished ---") # Changed

    # --- Load Results and Generate Plots ---
    print("--- Generating Final Plots and Metrics for Optimal KANN Model ---") # Changed
    ...
    print("\n--- Final Optimal KANN Model Performance Metrics ---") # Changed
    ...
    plt.title('Final Optimal KANN Model: Estimated vs. Actual SOH', fontsize=16) # Changed
    plt.xlabel('Cycle Number', fontsize=12)
    plt.ylabel('State of Health (SOH)', fontsize=12)
    plt.legend(fontsize=10)
    plt.grid(True)
    
    plot_path = os.path.join(final_save_folder, 'final_optimal_kann_soh_comparison_plot.png') # Changed
    plt.savefig(plot_path, dpi=300)
    print(f"Final SOH comparison plot for optimal KANN model saved to: {plot_path}") # Changed
    plt.close()

def main():
    parser = argparse.ArgumentParser(description='Analyze and Plot KANN Sensitivity Analysis Results and Train Optimal Model') # Changed
    parser.add_argument('--base_folder', type=str, 
                        default='optimized_plots/KANN_sensitivity_analysis', # Changed
                        help='Base folder containing sensitivity analysis results')
    parser.add_argument('--final_output_folder', type=str,
                        default='optimized_plots/KANN_Final_Optimized_Results', # Changed
    ...
    print("\n--- Identified Optimal Hyperparameters for KANN Model ---") # Changed
    for param, value in optimal_hyperparams.items():
        print(f"  {param}: {value}")
    print("-----------------------------------------------------\n")

    # Part 3: Train Final Optimal Model and Generate Final Plots/Metrics
    train_final_optimal_nn_model(optimal_hyperparams, args.final_output_folder) # Renamed function call
