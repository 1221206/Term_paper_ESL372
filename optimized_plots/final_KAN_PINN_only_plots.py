...
from Model.Model_KAN_PINN import PINN as Trainer
...
def get_args_for_trainer():
    ...
    parser.add_argument('--save_folder', type=str, default='optimized_plots/KAN_PINN_Final_Optimized_Results', help='save folder')
...
def plot_single_experiment_losses(experiment_folder, param_name, param_value):
    ...
    plt.title(f'KAN-PINN Sensitivity: {param_name} = {param_value} (Total Loss)', fontsize=16)
...
def analyze_and_plot_sensitivity_type(base_output_folder, sensitivity_type_folder, param_name):
    ...
    plt.title(f'KAN-PINN Sensitivity Analysis: Min Total Loss vs. {param_name}', fontsize=16)
...
def analyze_and_plot_2d_sensitivity(base_output_folder, sensitivity_type_folder, param_names):
    ...
    plt.title(f'KAN-PINN: Min Total Loss vs. {param_names[0]} and {param_names[1]}')
    analysis_results_folder = os.path.join(base_output_folder, 'analysis_results')
    os.makedirs(analysis_results_folder, exist_ok=True)
...
def train_final_optimal_kan_pinn_model(optimal_hyperparams, final_save_folder):
    print("\n--- Starting Final KAN-PINN Model Training with Optimal Hyperparameters ---")
    ...
    print("--- Final KAN-PINN Model Training Finished ---")
    ...
    plt.legend(fontsize=10)
    plt.grid(True)
    plot_path = os.path.join(final_save_folder, 'final_optimal_kan_pinn_soh_comparison_plot.png')
    plt.savefig(plot_path, dpi=300)
    plt.close()

def main():
    parser = argparse.ArgumentParser(description='Analyze KAN-PINN Sensitivity and Train Optimal Model')
    parser.add_argument('--base_folder', type=str, default='optimized_plots/KAN_PINN_sensitivity_analysis', help='Base folder for sensitivity results')
    parser.add_argument('--final_output_folder', type=str, default='optimized_plots/KAN_PINN_Final_Optimized_Results', help='Folder for final optimal model results')
    ...
    print("\n--- Identified Optimal Hyperparameters for KAN-PINN Model ---")
    for param, value in optimal_hyperparams.items():
        print(f"  {param}: {value}")
    print("-----------------------------------------------------\n")

    train_final_optimal_kan_pinn_model(optimal_hyperparams, args.final_output_folder)
