...
class WeightedAttentionLayer(nn.Module):
    def __init__(self, hidden_dim):
        ...
        # attn wt for each feature, updateable
        self.attn_weight = nn.Parameter(torch.randn(hidden_dim))  # (hidden_dim,) - one wt per feature

    def forward(self, x):
        # compute attn wt for each feature and apply
        # attn_weight: (hidden_dim,) x: (batch_size, seq_len, hidden_dim) multiplied via broadcasting
...
class KAN(nn.Module):
    def __init__(self, input_dim=17, output_dim=1, hidden_dim=60, num_hidden_layers=4, dropout=0.0):
        ...
        # add attn layer after hidden layer
...
class PINN(nn.Module):
    def __init__(self, args):
        ...
        if hasattr(args, 'save_folder') and args.save_folder and not os.path.exists(args.save_folder):
            os.makedirs(args.save_folder)
        
        log_dir = None
        if hasattr(args, 'log_dir') and args.log_dir is not None:
            if hasattr(args, 'save_folder') and args.save_folder is not None:
                log_dir = os.path.join(args.save_folder, args.log_dir)
            else:
        ...
        if hasattr(args, 'warmup_lr'):
            ...
            self.scheduler2 = LR_Scheduler(optimizer=self.optimizer2,
                                           warmup_epochs=args.warmup_epochs_F,
                                           warmup_lr=args.warmup_lr_F,
                                           num_epochs=args.epochs,
                                           base_lr=args.lr_F,
                                           final_lr=args.final_lr_F)
    ...
    def test_for_plotting(self, dataloader):
        self.eval()
        true_label = []
        pred_label = []
        with torch.no_grad():
    ...
    def train_one_epoch(self, epoch, dataloader):
        self.train()
        loss1_meter, loss2_meter, loss3_meter = AverageMeter(), AverageMeter(), AverageMeter()
    ...
    def Train(self, trainloader, testloader=None, validloader=None):
        ...
        best_epoch = 0
        best_metrics = {'MSE': 0, 'MAE': 0, 'MAPE': 0, 'RMSE': 0, 'R2': 0}

        for e in range(1, self.args.epochs + 1):
            early_stop += 1
            loss1, loss2, loss3 = self.train_one_epoch(e, trainloader)
            current_lr = self.scheduler.step()
            current_lr_F = self.scheduler2.step()
            total_loss = loss1 + self.alpha * loss2 + self.beta * loss3

            epoch_losses.append(total_loss)
            epoch_loss1.append(loss1)
            epoch_loss2.append(loss2)
            epoch_loss3.append(loss3)

            info = '[Train] epoch:{}, lr:{:.6f}, lr_F:{:.6f}, total loss:{:.6f}, data loss:{:.6f}, PDE loss:{:.6f}, physics loss:{:.6f}'.format(
                e, current_lr, current_lr_F, total_loss, loss1, loss2, loss3)
        ...
        if self.args.save_folder is not None:
            ...
            np.save(os.path.join(self.args.save_folder, 'epoch_loss1.npy'), epoch_loss1)
            np.save(os.path.join(self.args.save_folder, 'epoch_loss2.npy'), epoch_loss2)
            np.save(os.path.join(self.args.save_folder, 'epoch_loss3.npy'), epoch_loss3)
            np.save(os.path.join(self.args.save_folder, 'valid_mses.npy'), valid_mses)

            # save best performance hyper params 
            summary_path = os.path.join(self.args.save_folder, 'experiment_summary.txt')
            with open(summary_path, 'w') as f:
                ...
                f.write(f"  MSE: {best_metrics['MSE']:.8f}\n")
...
if __name__ == "__main__":
    ...
    def get_args():
        ...
        parser.add_argument('--warmup_epochs_F', type=int, default=10, help='warmup epoch of F')
        parser.add_argument('--warmup_lr_F', type=float, default=5e-3, help='warmup lr of F')
        parser.add_argument('--final_lr_F', type=float, default=1e-3, help='final lr of F')
