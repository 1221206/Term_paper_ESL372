...
class WeightedAttentionLayer(nn.Module):
    def __init__(self, hidden_dim, layer_id=0):
        ...
        # The weight is a learnable parameter representing the attention weight for each input feature
        self.attn_weight = nn.Parameter(torch.randn(hidden_dim))  # (hidden_dim,) - one wt per feature
        self.layer_id = layer_id  # Records the ID of the layer, used for saving weights
        self.feature_before = None  # Stores the features before applying attention
        self.feature_after = None  # Stores the features after applying attention
        self.current_weights = None  # Stores the current weight values

    def forward(self, x):
        ## Store the features before applying attention
        self.feature_before = x.detach().cpu().mean(dim=0).numpy()  # 取批次平均

        # Compute attention weights for each feature
        # attn_weight: (hidden_dim,) and x: (batch_size, seq_len, hidden_dim) are multiplied via broadcasting
   
        weight = torch.sigmoid(self.attn_weight)  
        self.current_weights = weight.detach().cpu().numpy()  # store curretn wt

        weighted_x = x * weight  # apply wt to feature

        # store features after attention
        self.feature_after = weighted_x.detach().cpu().mean(dim=0).numpy()  # 取批次平均

        return weighted_x

    def save_attention_info(self, save_dir):
        #Save attention mechanism information
    ...
    def _visualize_attention(self, save_dir):
        # visualize attn wt and effects
        fig, axes = plt.subplots(3, 1, figsize=(10, 15))

        # plt featues before attn
...
class KAN(nn.Module):
    def __init__(self, input_dim=17, output_dim=1, hidden_dim=60, num_hidden_layers=4, dropout=0.0):
        ...
        self.attention_layers = []  # store all atention layers

        # hidden layers
        layers = [nn.Linear(input_dim, hidden_dim), nn.ReLU()]

        # add weighted attn layer after hidden layer
        for i in range(num_hidden_layers - 1):
            layers.append(nn.Linear(hidden_dim, hidden_dim))
            layers.append(nn.ReLU())
            attention_layer = WeightedAttentionLayer(hidden_dim, layer_id=i)  # weighted layer
    ...
    def save_attention_data(self, save_dir):
        # save data from all attn layers
...
class Solution_u(nn.Module):
    ...
    def save_attention_data(self, save_dir):
        # save data from attn layer in encoder
...
class PINN(nn.Module):
    def __init__(self, args):
        ...
        self.l2_lambda = args.l2_lambda  # adjust default value as needed
    ...
    def train_one_epoch(self, epoch, dataloader):
        ...
        for idx, (x1, x2, y1, y2) in enumerate(dataloader):
            ...
            # lambda1, lambda2, lambda3 for diff loss component
    ...
    def Train(self, trainloader, testloader=None, validloader=None):
        ...
        for e in range(1, self.args.epochs + 1):
            ...
            # record changes in attn mech after each epoch
            ...
            if valid_mse < min_valid_mse and testloader is not None:
                ...
                if self.args.save_folder is not None:
                    ...
                    # best attn weights for best model
    ...
    def _plot_attention_weights_evolution(self):
        # attn weights accross all epochs
