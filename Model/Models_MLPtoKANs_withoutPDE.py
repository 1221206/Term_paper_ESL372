...
class PINN(nn.Module):
    ...
    def train_one_epoch(self, epoch, dataloader):
        self.train()
        loss1_meter, loss3_meter, l2_reg_meter = AverageMeter(), AverageMeter(), AverageMeter()

        for idx, (x1, x2, y1, y2) in enumerate(dataloader):
            x1, x2, y1, y2 = x1.to(device), x2.to(device), y1.to(device), y2.to(device)
            u1, _ = self.forward(x1)
            u2, _ = self.forward(x2)
            ...
            # final loss
            loss = self.lambda1 * loss1 + self.lambda3 * loss3 + l2_reg
            ...
            if (idx + 1) % 50 == 0:
                print(f"[epoch:{epoch} iter:{idx + 1}] data loss:{loss1_meter.avg:.6f}, "
                      f"physics loss:{loss3_meter.avg:.6f}, L2 loss:{l2_reg.item():.6f}")

        return loss1_meter.avg, 0.0, loss3_meter.avg, l2_reg_meter.avg
    ...
    def Train(self, trainloader, testloader=None, validloader=None):
        ...
        for e in range(1, self.args.epochs + 1):
            early_stop += 1
            loss1, _, loss3, l2_reg = self.train_one_epoch(e, trainloader)
            current_lr = self.scheduler.step()
            total_loss = self.lambda1 * loss1 + self.lambda3 * loss3 + self.l2_lambda * l2_reg
