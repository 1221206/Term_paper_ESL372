...
class PINN(nn.Module):
    def __init__(self, args):
        ...
        # Handle args.log_dir and args.save_folder being None
        log_dir_path = None
        if hasattr(args, 'save_folder') and args.save_folder:
            if not os.path.exists(args.save_folder):
                os.makedirs(args.save_folder)
            if hasattr(args, 'log_dir') and args.log_dir:
                log_dir_path = os.path.join(args.save_folder, args.log_dir)
            else:
                log_dir_path = os.path.join(args.save_folder, 'logging.txt') # Default log file name
        elif hasattr(args, 'log_dir') and args.log_dir:
            log_dir_path = args.log_dir
        
        # Initialize logger only if log_dir_path is not None
        if log_dir_path:
            self.logger = get_logger(log_dir_path)
            self._save_args()
        else:
            self.logger = None # No logger if no log_dir

        self.solution_u = Solution_u(u_hidden_dim=args.u_hidden_dim, u_layers_num=args.u_layers_num).to(device)
        self.dynamical_F = KAN(input_dim=35, output_dim=1, hidden_dim=args.F_hidden_dim, num_hidden_layers=args.F_layers_num, dropout=0.0).to(device)
        ...
        self.l2_lambda = args.l2_lambda  # default val
    ...
    def test_for_plotting(self, testloader):
        self.eval()
        true_labels = []
        pred_labels = []
        with torch.no_grad():
            for x_batch, _, y_batch, _ in testloader:
                x_batch = x_batch.to(device)
                u_pred = self.predict(x_batch)
                true_labels.append(y_batch.cpu().numpy())
                pred_labels.append(u_pred.cpu().numpy())

        true_labels = np.concatenate(true_labels, axis=0)
        pred_labels = np.concatenate(pred_labels, axis=0)

        # Calculate metrics
        mae, mape, mse, rmse, r2 = eval_metrix(pred_labels, true_labels)
        metrics = {
            'MAE': mae,
            'MAPE': mape,
            'MSE': mse,
