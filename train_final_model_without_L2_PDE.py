...
def main():
    ...
    print("\n--- Generating Final Plots and Metrics for Final Model ---")
    ...
    print("\n--- Final Optimal Model Performance Metrics ---")
    ...
    plt.title('Final Optimal Model: Estimated vs. Actual SOH', fontsize=16)
    plt.xlabel('Cycle Number', fontsize=12)
    plt.ylabel('State of Health (SOH)', fontsize=12)
    plt.legend(fontsize=10)
    plt.grid(True)
    
    plot_path = os.path.join(args.save_folder, 'final_optimal_model_soh_comparison_plot.png')
    plt.savefig(plot_path, dpi=300)
    print(f"Final SOH comparison plot for optimal model saved to: {plot_path}")
    plt.close()
